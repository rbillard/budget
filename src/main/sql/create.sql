...
create table T_BUDGET (id bigint NOT NULL AUTO_INCREMENT, label varchar(255) not null, T_USER_ID bigint not null, primary key (id));
create table T_OPERATION (id bigint NOT NULL AUTO_INCREMENT, amount numeric(19,2) not null, date timestamp not null, label varchar(255) not null, T_BUDGET_ID bigint not null, T_PERIOD_ID bigint not null, primary key (id));
...
alter table T_OPERATION add constraint FK_mql94xv969ok744ldtx8ku9n0 foreign key (T_BUDGET_ID, T_PERIOD_ID) references L_PERIOD_BUDGET (T_BUDGET_ID, T_PERIOD_ID);
alter table T_PERIOD add constraint FK_oh1t92uaipj8i65pgf7g2f1ex foreign key (T_USER_ID) references T_USER ( ID );
alter table T_BUDGET add constraint FK_jdgkjdgsjg5dkjg5djs6fjkd5 foreign key (T_USER_ID) references T_USER ( ID );